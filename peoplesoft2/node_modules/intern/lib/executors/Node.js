(function (factory) {
    ...
    var Node = (function (_super) {
            ...
            process.on('uncaughtException', function (reason) {
                ...
                if (_this.config.warnOnUncaughtException &&
                    (_this.config.warnOnUncaughtException === true ||
                        _this.config.warnOnUncaughtException.test("" + reason))) {
                    _this.emit('warning', "" + reason);
                }
                else {
                    _this.emit('error', reason);
        ...
        Node.prototype._resolveConfig = function () {
            ...
            return _super.prototype._resolveConfig.call(this).then(function () {
                ...
                ['basePath', 'internPath'].forEach(function (property) {
                    config[property] = path_2.normalizePathEnding(path_1.resolve(config[property]), path_1.sep);
                });
                ...
                ['serverUrl', 'functionalBaseUrl'].forEach(function (property) {
                    if (config[property]) {
                ...
                if (config.tunnel === 'selenium') {
                    var driverNames_1 = _this._getSeleniumDriverNames();
                    var tunnelOptions = config.tunnelOptions;
                    if (tunnelOptions.drivers) {
                        tunnelOptions.drivers
                            ...
                            var index = driverNames_1.indexOf(name);
                            if (index !== -1) {
                                driverNames_1.splice(index, 1);
                            }
                        });
                        tunnelOptions.drivers = tunnelOptions.drivers.concat(driverNames_1.map(function (name) { return ({ name: name }); }));
                    }
                    else {
                        tunnelOptions.drivers = driverNames_1.map(function (name) { return ({ name: name }); });
                    }
                }
            });
        };
        Node.prototype._getSeleniumDriverNames = function () {
            var config = this.config;
            var browserNames = config.environments
                .map(function (env) { return env.browserName; })
            ...
            return Object.keys(browserNames.reduce(function (allNames, name) {
                var _a;
                return (tslib_1.__assign({}, allNames, (_a = {}, _a[name] = true, _a)));
            }, {}));
        };
        ...
        Node.prototype._setInstrumentationHooks = function () {
            var _this = this;
            istanbul_lib_hook_1.hookRunInThisContext(function (filename) { return _this.shouldInstrumentFile(filename); }, function (code, _a) {
                var filename = _a.filename;
                return _this.instrumentCode(code, filename);
            });
            this._unhookRequire = istanbul_lib_hook_1.hookRequire(function (filename) { return _this.shouldInstrumentFile(filename); }, function (code, _a) {
                var filename = _a.filename;
                return _this.instrumentCode(code, filename);
            });
        };
