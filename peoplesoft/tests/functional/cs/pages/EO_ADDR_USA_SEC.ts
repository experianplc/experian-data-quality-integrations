  ...
  return function() {
    return this.parent
      .execute(function(GLOBAL_INTUITIVE_AUTH_TOKEN, root) {
        ...
        element.id = "edq-9.2-cs-pages-EO_ADDR_USA_SEC";
        element.setAttribute("GLOBAL_INTUITIVE_AUTH_TOKEN", GLOBAL_INTUITIVE_AUTH_TOKEN);

        let script = document.createElement("script");
        script.src = `http://localhost:8000/lib/9.2/cs/pages/EO_ADDR_USA_SEC/integration.js`;
...
/**
 * This URL will be used to get to the relevant component
 */
const URL = "http://bospsoftcs92.qas.com:8000/psc/ps/EMPLOYEE/SA/c/PROCESS_APPLICATIONS.SCC_BIO_DEMO.GBL";

registerSuite("CS - EO_ADDR_USA_SEC Tests", {
  before: function() {
    return this.remote
  ...
  beforeEach: function() {
    ...
    return this.remote  
      ...
      .then(pollUntil(function() {
        return document.querySelector(".PSPUSHBUTTONTBADD");
      }))
      .findByCssSelector(".PSPUSHBUTTONTBADD")
        .click()
        .end()
      .then(pollUntil(function() {
          return document.querySelector("[id^=DERIVED_ADDR_UPDATE_ADDRESS]");
      }))
      .findByCssSelector("[id^=DERIVED_ADDR_UPDATE_ADDRESS]")
        .click()
        .end()
      .then(pollUntil(function() {
        return document.getElementById("win0divDERIVED_ADDRESS_STATE");
      }))
      .then(pollUntil(function() { 
        window.EDQ = null;
        ...
        if (document.getElementById("edq-9.2-cs-pages-EO_ADDR_USA_SEC")) {
          let integration = document.getElementById("edq-9.2-cs-pages-EO_ADDR_USA_SEC");
          integration.remove();
        }
  ...
  tests: {
    "Pro Web without adding integration fails": function() {
      return this.remote
        ...
        .then(pollUntil(function() {
          return (document.querySelector("#DERIVED_ADDRESS_CITY") as HTMLInputElement).value;
        }))
